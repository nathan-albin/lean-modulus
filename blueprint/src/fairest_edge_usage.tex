...
0$ for some law $\mu^*$ optimal for $\FEU(\Gamma_G)$. The set of fair trees is
...
\subsection{Strong duality for 2-modulus}

The general existence/uniqueness and Fulkerson duality theorems of
Section~\ref{chap:common} (Theorems~\ref{thm:modulus-extremal-unique} and
\ref{thm:modulus-fulkerson-duality}) are stated for general $1 < p < \infty$
but are not yet formalized in that generality. The results below are the
$p = 2$ specializations actually used by this chapter; they are proved
directly using the connection between the two-norm and the inner product.

\begin{theorem}[Existence and uniqueness of the 2-extremal density]
\label{thm:extremal-unique-two}
\uses{def:extremal-density}
Let $\Gamma$ be a finite family of objects on $G = (V, E, \sigma)$ with
$\Adm(\Gamma) \ne \emptyset$. There is a unique density $\rho^* \in
\Adm(\Gamma)$ achieving $\Mod_{2,\sigma}(\Gamma)$.
\end{theorem}

\begin{theorem}[Strong duality for 2-modulus]
\label{thm:strong-duality-two}
\uses{def:admissible-density, thm:extremal-unique-two}
Let $\Gamma$ be a finite family of objects on $G = (V, E, \sigma)$ with
$\Adm(\Gamma) \ne \emptyset$, and let $\rho^*$ be the extremal density for
$\Mod_{2,\sigma}(\Gamma)$. There exists $\lambda^* \in \mathbb R_{\ge
0}^\Gamma$ such that
\[
  \rho^*(e) = \frac{1}{2\sigma(e)} \sum_{\gamma \in \Gamma} \gamma(e)
  \lambda^*(\gamma) \qquad \forall e \in E,
\]
\[
  \lambda^*(\gamma) \bigl(1 - \ell_{\rho^*}(\gamma)\bigr) = 0 \qquad \forall
  \gamma \in \Gamma,
\]
and
\[
  \Mod_{2,\sigma}(\Gamma) = \sum_{\gamma \in \Gamma} \lambda^*(\gamma) -
  \frac14 \sum_{e \in E} \frac{1}{\sigma(e)} \Bigl(\sum_{\gamma \in \Gamma}
  \gamma(e) \lambda^*(\gamma)\Bigr)^{\!2}.
\]
\end{theorem}

\begin{corollary}[Weak duality for the Lagrangian dual]
\label{cor:weak-duality-two}
\uses{def:admissible-density, def:energy}
For any $\rho \in \Adm(\Gamma)$ and any $\lambda \in \mathbb R_{\ge
0}^\Gamma$,
\[
  \Energy_{2,\sigma}(\rho) \ \ge\ \sum_{\gamma \in \Gamma} \lambda(\gamma) -
  \frac14 \sum_{e \in E} \frac{1}{\sigma(e)} \Bigl(\sum_{\gamma \in \Gamma}
  \gamma(e) \lambda(\gamma)\Bigr)^{\!2},
\]
with equality if and only if $\rho$ and $\lambda$ satisfy the stationarity
and complementary slackness conditions of
Theorem~\ref{thm:strong-duality-two}.
\end{corollary}

\begin{corollary}[2-modulus Fulkerson duality]
\label{cor:fulkerson-duality-two}
\uses{thm:strong-duality-two, def:fulkerson-dual}
In the setting of Theorem~\ref{thm:strong-duality-two}, define $\eta^*(e) :=
\sigma(e) \rho^*(e) / \Mod_{2,\sigma}(\Gamma)$ for $e \in E$. Then $\eta^*
\in \Adm(\widehat\Gamma)$ and $\eta^*$ is extremal for
$\Mod_{2,\sigma^{-1}}(\widehat\Gamma)$, with
\[
  \Mod_{2,\sigma}(\Gamma) \, \Mod_{2,\sigma^{-1}}(\widehat\Gamma) = 1.
\]
This recovers the $p = q = 2$ case of
Theorem~\ref{thm:modulus-fulkerson-duality} without the general machinery.
\end{corollary}
...
Let $\Gamma_G$ be the (finite) family of spanning trees of $G = (V, E)$, with
usage given by the indicator vectors $\mathbf 1_{\{e \in \gamma\}}$. The Fulkerson
...
\uses{def:modulus, def:meo, def:feu, thm:strong-duality-two}
Let $\Gamma = \Gamma_G$ be the (finite) family of spanning trees of $G = (V, E)$ and let
...
and only if
\[
  \rho \in \Adm(\Gamma), \quad \eta(e) = \sum_{\gamma \in \Gamma} \gamma(e) \, \mu(\gamma) \ \forall e \in E, \quad
